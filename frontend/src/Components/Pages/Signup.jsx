...
function Signup(){
  ...
  const {t, i18n} = useTransition(["gallaryinfo"]);
  ...
  const  handleSubmit = async () => {
    if (name == "" || username == "" || email == "" || password == "") {
      ...
      axios
        .post("http://172.20.10.10:8080/api/gallaries", item)
        .then((response) => {
          setPending(true);
          localStorage.setItem("user", JSON.stringify(response.data));   {/*This line stores the data returned from the server (response.data)
                                                                            in the browser's local storage under the key "user".
                                                                            JSON.stringify() is used to convert the JavaScript object into a JSON string
                                                                            before storing it, as local storage can only store strings.*/}
    ...
    <>
       {orientation =="landscape-primary" ||
        orientation == "landscape-secondary" ? (
          ...
          <>
            <ReturnButton path="/" />
            <div style={{ alignItems: "center", maginTop: "60px" }}>
              {/*------------------------------------ERORR-------------------------------------- */}
              <Input
              ...
              { error != "" && (
                <Text maginTop={"25px"} color={"red-text"} text={error} />
              )}
